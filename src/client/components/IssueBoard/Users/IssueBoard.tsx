...
export default function IssueBoard() {
  const { data, error, addIssue, searchOptions, token, getNewSearch } =
    useIssues();
  ...
  useEffect(() => {
    if (addIssue.isSuccess) {
      ...
      setLocalError(null);
      setAction(
        "Your suggestion/issue was sent successfully! Later on maybe I read it.",
      );
      setTimeout(() => {
        setAction(null);
      }, 3000);
  ...
  const handleNewIssue = (e: FormEvent<HTMLFormElement>) => {
    ...
    else setLocalError("Please don't leave required fields empty.");
  };
  ...
  return (
    <div>
      ...
      <form onSubmit={handleNewIssue}>
        <label htmlFor="">
          Project*:{" "}
          <input type="text" value={project} onChange={handleProject} />
        </label>
        <label htmlFor="">
